...
export default function AddEquipmentPage() {
  ...
  // NEW: State for the Hybrid Tracking System
  ...
  const handleAddEquipment = async (e) => {
    ...
    try {
      if (trackingType === 'bulk') {
        // BULK METHOD: Create ONE document for the entire quantity
        ...
        // INDIVIDUAL METHOD: Create a separate document for every single item
      ...
      console.error(error); // This will log the actual error if it fails again
  ...
  const groupedInventory = Object.values(equipmentList.reduce((acc, item) => {
    ...
    // Check available logic depending on if it's bulk or individual
    ...
    <div className={`min-h-screen w-full p-6 md:p-12 lg:p-16 flex flex-col items-center overflow-y-auto relative transition-colors duration-500 ${pageBg}`} style={{ fontFamily: "ui-monospace, monospace" }}>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6">
          ...
          <div className={`relative w-full max-w-md p-10 backdrop-blur-3xl border rounded-[2.5rem] shadow-2xl text-center ${isDarkMode ? 'bg-gradient-to-br from-white/10 to-[#050B14] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            <p className={`mb-8 leading-relaxed ${isDarkMode ? 'text-white/50' : 'text-slate-500'}`}>This action is permanent. Are you sure you want to remove this asset tag from the inventory?</p>
        ...
        <form onSubmit={handleAddEquipment} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
          ...
          <div className="lg:col-span-12 flex flex-col md:flex-row gap-4 mb-4">
            <div
              ...
              className={`flex-1 p-6 rounded-3xl border cursor-pointer transition-all ${trackingType === 'individual' ? `border-[#3852A4] shadow-[0_0_15px_rgba(56,82,164,0.15)] ${isDarkMode ? 'bg-[#3852A4]/10 text-blue-100' : 'bg-blue-50/50 text-[#3852A4]'}` : inputBg}`}
            >
              <h4 className="font-bold uppercase tracking-widest text-sm mb-1">Individual Tracking</h4>
              <p className={`text-[10px] uppercase tracking-widest ${trackingType === 'individual' ? 'opacity-80' : 'opacity-40'}`}>Generates a unique QR code for every single unit.</p>
            </div>

            <div
              onClick={() => setTrackingType('bulk')}
              className={`flex-1 p-6 rounded-3xl border cursor-pointer transition-all ${trackingType === 'bulk' ? `border-[#3852A4] shadow-[0_0_15px_rgba(56,82,164,0.15)] ${isDarkMode ? 'bg-[#3852A4]/10 text-blue-100' : 'bg-blue-50/50 text-[#3852A4]'}` : inputBg}`}
            >
              <h4 className="font-bold uppercase tracking-widest text-sm mb-1">Bulk Tracking</h4>
              <p className={`text-[10px] uppercase tracking-widest ${trackingType === 'bulk' ? 'opacity-80' : 'opacity-40'}`}>Generates 1 single QR code for the entire quantity.</p>
            </div>
          </div>
      ...
      <div className={`w-full max-w-6xl border rounded-[3rem] overflow-hidden shadow-xl ${isDarkMode ? 'bg-white/[0.02] border-white/10' : 'bg-white border-slate-200'}`}>
        <div className={`p-10 border-b flex justify-between items-center ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
          ...
          <span className={`text-sm font-bold uppercase tracking-widest ${countText}`}>{equipmentList.length} Total Database Entries</span>
        </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className={`divide-y ${isDarkMode ? 'divide-white/5' : 'divide-slate-100'}`}>
            {groupedInventory.map((group) => {
              ...
              // FLAT ROW: IF ONLY 1 ENTRY EXISTS (Single Item OR 1 Bulk Package)
                ...
                return (
                  <tr key={item.id} className={`transition-all ${isDarkMode ? 'hover:bg-white/[0.02]' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className="px-10 py-8 text-xl font-bold">
                      {editingId === item.id ? (
                        <input autoFocus value={editName} onChange={(e) => setEditName(e.target.value)} onBlur={() => handleUpdate(item.id)} onKeyDown={(e) => e.key === 'Enter' && handleUpdate(item.id)} className={`border border-[#3852A4] outline-none px-4 py-2 rounded-xl w-full ${editInput}`} />
                      ) : item.name}
                    </td>
                    <td className="px-10 py-8 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${item.status === 'available' ? badgeAvailable : badgeUnavailable}`}>
                        {isBulk ? `${item.availableQuantity} / ${item.totalQuantity} Available` : item.status}
                      </span>
                    </td>
                    <td className="px-10 py-8 text-center">
                      <div className="bg-white p-2 rounded-xl inline-block shadow-sm relative group">
                        <QRCodeSVG value={`${window.location.origin}/borrow/${item.id}`} size={64} level={"H"} includeMargin={false} />
              ...
              // FOLDER: IF MULTIPLE INDIVIDUAL ITEMS EXIST UNDER SAME NAME
              return (
                <React.Fragment key={group.key}>
                  <tr onClick={() => toggleGroup(group.key)} className={`cursor-pointer transition-all ${isDarkMode ? 'bg-white/[0.05] hover:bg-white/[0.08]' : 'bg-slate-100 hover:bg-slate-200'}`}>
                    ...
                    <td className="px-10 py-6 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${group.availableCount > 0 ? badgeAvailable : badgeUnavailable}`}>
                        {group.availableCount} / {group.items.length} Available
                      </span>
                    </td>
                  ...
                  {expandedGroups[group.key] && group.items.map((item) => (
                    <tr key={item.id} className={`transition-all border-l-4 border-l-[#3852A4] ${isDarkMode ? 'bg-black/30 hover:bg-black/50' : 'bg-slate-50/50 hover:bg-slate-50'}`}>
                      ...
                      <td className="px-10 py-8 text-lg font-medium opacity-80">
                        {editingId === item.id ? (
                          <input autoFocus value={editName} onChange={(e) => setEditName(e.target.value)} onBlur={() => handleUpdate(item.id)} onKeyDown={(e) => e.key === 'Enter' && handleUpdate(item.id)} className={`border border-[#3852A4] outline-none px-4 py-2 rounded-xl w-full ${editInput}`} />
                        ) : item.name}
                      </td>
                      <td className="px-10 py-8 text-center">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${item.status === 'available' ? badgeAvailable : badgeUnavailable}`}>{item.status}</span>
                      </td>
                      <td className="px-10 py-8 text-center">
                        <div className="bg-white p-2 rounded-xl inline-block shadow-sm">
                          <QRCodeSVG value={`${window.location.origin}/borrow/${item.id}`} size={64} level={"H"} includeMargin={false} />
                        </div>
                        <div className="mt-2"><a href={`/borrow/${item.id}`} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-blue-400 hover:underline uppercase tracking-widest cursor-pointer">Test Link</a></div>
                      </td>
                      <td className="px-10 py-8 text-right space-x-6">
                        <button onClick={() => { setEditingId(item.id); setEditName(item.name); }} className="text-blue-400 hover:text-blue-300 font-bold text-sm uppercase tracking-widest transition-all cursor-pointer">Edit</button>
                        <button onClick={() => setDeleteModal({ show: true, itemId: item.id })} className="text-red-500/60 hover:text-red-400 font-bold text-sm uppercase tracking-widest transition-all cursor-pointer">Delete</button>
