...
export default function DashboardPage() {
  ...
  return (
    <div className={`min-h-screen md:h-screen w-full flex flex-col p-4 sm:p-6 md:p-10 lg:p-14 transition-colors duration-500 relative ${isDarkMode ? 'bg-[#050B14] text-white' : 'bg-slate-50 text-slate-900'}`} style={{ fontFamily: "'Google Sans', 'Product Sans', 'Segoe UI', system-ui, sans-serif" }}>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-md p-8 border rounded-[2rem] shadow-2xl text-center ${isDarkMode ? 'bg-[#050B14] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            <p className="mb-8 text-sm sm:text-base opacity-50">End your current session?</p>
            <div className="flex gap-4">
              <button onClick={() => setShowLogoutModal(false)} className={`flex-1 py-3 sm:py-4 rounded-xl sm:rounded-2xl border transition-all font-bold ${isDarkMode ? 'border-white/10 hover:bg-white/5' : 'border-slate-200 hover:bg-slate-100 text-slate-700'}`}>Cancel</button>
              <button onClick={handleLogout} className="flex-1 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-slate-900 text-white font-bold shadow-lg active:scale-95 transition-all">Sign out</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={`md:hidden fixed inset-0 z-[90] transition-all duration-300 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-[80%] max-w-[300px] p-8 flex flex-col shadow-2xl transition-transform duration-500 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} ${isDarkMode ? 'bg-[#050B14] border-l border-white/10' : 'bg-white border-l border-slate-200'}`}>
          <div className="flex justify-between items-center mb-10">
            <span className="text-xs font-bold uppercase tracking-wide opacity-50">Menu</span>
            <button onClick={() => setIsMenuOpen(false)} className="p-2 opacity-50 transition-opacity hover:opacity-100"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-6 border-t border-white/5">
            <button onClick={toggleTheme} className={`flex items-center justify-between py-4 px-6 rounded-2xl border transition-all ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
            ...
            <button onClick={() => { setIsMenuOpen(false); setShowLogoutModal(true); }} className="py-4 rounded-2xl font-bold bg-red-500/10 text-red-400 transition-colors hover:bg-red-500/20">Sign out</button>
          </div>
        </div>
      </div>
      ...
      <header className="flex justify-between items-center mb-8 md:mb-14">
        <div>
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-normal mb-1 sm:mb-2">Overview</h1>
          <p className="text-xs sm:text-sm font-bold uppercase tracking-wide opacity-50">Lab Inventory System</p>
        </div>

        {/* Desktop Only Buttons */}
        <div className="hidden md:flex items-center gap-4">
          <button onClick={toggleTheme} className={`p-3 rounded-full border transition-all cursor-pointer ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white border-slate-200 shadow-sm hover:bg-slate-50'}`}>
        ...
        <button onClick={() => setIsMenuOpen(true)} className={`md:hidden p-3 rounded-full border transition-colors ${isDarkMode ? 'border-white/10 bg-white/5 hover:bg-white/10' : 'border-slate-200 bg-white hover:bg-slate-50'}`}>
      ...
      <div className="hidden md:block mt-14">
        <h2 className="text-sm sm:text-base font-bold uppercase tracking-wide mb-8 opacity-50 text-center md:text-left">Management Controls</h2>
